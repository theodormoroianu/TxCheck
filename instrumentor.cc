                           ...
                           shared_ptr<schema> db_schema_) : db_schema(db_schema_)
{
    ...
    cerr << "Printing update changes: " << endl;
    for (int i = 0; i < (int)final_stmt_queue.size(); i++)
    {
        auto update = dynamic_pointer_cast<update_stmt>(final_stmt_queue[i]);
        if (!update)
            continue;
        cerr << i << ":\n";
        auto &lst = update->set_list;
        for (int j = 0; j < (int)lst->value_exprs.size(); j++)
        {
            {
                cerr << "UPDATE " << lst->names[j] << ": " << *lst->value_exprs[j] << endl;
            }
            cerr << "\n\n";
        }
    }
