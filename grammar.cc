    ...
    : table_ref(p), is_lateral(lateral)
{
    ...
    relation_t *aliased_rel = &query->select_list->derived_table;
...
select_list::select_list(prod *p,
                         ...
                         bool select_all) : prod(p), prefer_refs(refs)
{
    ...
    if (select_all)
    { // pointed_type is null and prefer_refs is not null
...
query_spec::query_spec(prod *p,
                       ...
                       bool lateral,
                       vector<sqltype *> *pointed_type,
                       bool txn_mode) : prod(p), myscope(s)
{
...
query_spec::query_spec(prod *p, struct scope *s,
                       table *from_table,
                       shared_ptr<bool_expr> where_search) : prod(p), myscope(s)
{
    scope = &myscope; // isolate the scope, dont effect the upper ones
    scope->tables = s->tables;
    has_group = false;
...
query_spec::query_spec(prod *p, struct scope *s,
                       ...
                       shared_ptr<value_expr> right_operand) : prod(p), myscope(s)
{
    scope = &myscope; // isolate the scope, dont effect the upper ones
    scope->tables = s->tables;
    has_group = false;
    ...
    : prod(parent), myscope(s)
{
    ...
    do
    {
        ...
        relation_t *relation = &query->select_list->derived_table;
    ...
    : prod(p), myscope(s), modified_select_list(select_list)
{
    ...
    size_t try_time = 0;
    while (1)
    { // only choose column_reference as target ref
        if (auto columns_ref = dynamic_pointer_cast<column_reference>(select_exprs[chosen_index]))
        {
            ...
            {
                if (columns_ref->table_ref == r->ident())
                { // the table from from_clause
                    suitable_one = true;
                    break;
                }
            }
...
alter_table_stmt::alter_table_stmt(prod *parent, struct scope *s) : prod(parent), myscope(s)
{
    scope = &myscope;
    scope->tables = s->tables;
    ...
    { // rename column
        ...
        stmt_string = "alter table " + table_ref->ident() + " rename column " + column_ref->name + " to " + new_column_name;
    }
    else if (stmt_type == 2)
    { // add column
        auto new_column_name = "c_" + random_identifier_generate();
        ...
        stmt_string = "alter table " + table_ref->ident() + " add column " + new_column_name + " " + type->name;
    }
...
drop_table_stmt::drop_table_stmt(prod *parent, struct scope *s) : prod(parent), myscope(s)
{
    scope = &myscope;
    scope->tables = s->tables;
...
named_window::named_window(prod *p, struct scope *s) : prod(p), myscope(s)
{
    scope = &myscope;
    scope->tables = s->tables;
...
unioned_query::unioned_query(prod *p, struct scope *s, bool lateral, vector<sqltype *> *pointed_type) : prod(p), myscope(s)
{
    scope = &myscope; // isolate the scope, dont effect the upper ones
    scope->tables = s->tables;
    if (lateral)
...
{
    try
    { // only use insert_stmt to add data to target table
        s->new_stmt();
